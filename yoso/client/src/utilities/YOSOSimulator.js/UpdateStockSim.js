...
export default async function UpdateStock(purchases, time) {
  ...
  if (purchases.length > 0) {
    ...
    if (length > 1) {
      const daysBetween = moment(time).diff(
        moment(purchases[length - 1].simDate),
        "days"
      );
    ...
    console.log(
      `purchases[length-1].expiration: ${
        purchases[length - 1].expiration
      }; simDate = ${purchases[length - 1].simDate}; frequency is ${
    ...
    if (purchases[length - 1].expiration !== undefined) {
      ...
      const lastPurchaseDate = purchases[length - 1].simDate;
