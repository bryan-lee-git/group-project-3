    ...
    return (
      <div className="center-align">
        ...
        <Card className="z-depth-5 rounded animate-up">
          <h4>
            Click the <Icon>check_circle_outline</Icon> to Add or Remove from
            Cart
          </h4>
          <Input
              ...
              <Table className="striped highlight centered responsive-table">
                ...
                <tbody>
                  {this.state.items.map((item, index) => (
                    <tr
                      key={index}
                      className={item.purchased ? "strikethrough" : ""}
                    >
                      {item.purchased ? (
                        <td onClick={() => this.toggleCartItems(index)}>
        ...
        <Row>
          <Col s={12}>
            <Modal
              trigger={
                <Button className="btn-large animate-up-3 end-shop-btn">
                  DONE SHOPPING
                </Button>
              }
            >
              <Row>
                <Col s={12}>
                  <h4 className="center-align">
                    NICE JOB! NOW CHOOSE AN OPTION BELOW.
                  </h4>
                </Col>
              </Row>
              <Row>
                <Col s={6}>
                  <Button
                    onClick={e => this.endShopping(e, this.props.user.id)}
                    className="btn-large end-shop-btn"
                  >
                    END + SAVE
                  </Button>
                </Col>
                <Col s={6}>
                  <Button
                    onClick={e =>
                      this.endAndDelete(e, this.props.user.id, this.props.list)
                    }
                    className="btn-large end-shop-btn"
                  >
                    END + DELETE
